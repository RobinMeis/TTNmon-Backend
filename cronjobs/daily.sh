...
shopt -s nullglob
mysqldump --defaults-file=/home/www-data/.my.cnf ttnmon packets gateways networks gateway_list | gzip $
rm /var/www/html/api/dumps/ttnmon_dump-latest.sql.gz
ln -s  /var/www/html/api/dumps/ttnmon_dump-$(date +%F).sql.gz /var/www/html/api/dumps/ttnmon_dump-late$
